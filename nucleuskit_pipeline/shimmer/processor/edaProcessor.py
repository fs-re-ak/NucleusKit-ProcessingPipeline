...
from nucleuskit_pipeline.shimmer.localTools.cvxEDA import cvxEDA
...
def _detect_scr_events(phasic_down, gap_mask_down=None, fs=CVXEDA_RATE):
    ...
    if gap_mask_down is not None and np.any(gap_mask_down):
        peaks = peaks[~gap_mask_down[peaks]]
...
def computeArousal(recPath, show=False):
    ...
    try:
        ...
        scr_times = _detect_scr_events(phasic_down, gap_mask_down=gap_mask_down)
